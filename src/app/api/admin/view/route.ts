...
// brief and the domain reports that fed it.
//
...
export async function GET(req: NextRequest) {
  ...
  const [identities, briefs, weeklyBrief, domainReports] = await Promise.all([
  ...
  if (req.nextUrl.searchParams.get('format') === 'json') {
    return NextResponse.json({ profile: { id: profile.id, userName: profile.userName }, identities, briefs, weeklyBrief, domainReports })
  }

  const active = MODALITIES
